...
const METRICS = [
  { label: "active projects", value: "12", delta: "+3 today", tone: "var(--te-green)" },
  { label: "messages", value: "3,842", delta: "+11%", tone: "var(--te-blue)" },
  { label: "alerts", value: "2", delta: "review now", tone: "var(--te-red)" },
  { label: "deploy status", value: "healthy", delta: "all systems", tone: "var(--te-green)" },
];

const ACTIVITY_BARS = [42, 58, 49, 61, 55, 73, 67, 79, 71, 64, 69, 81];

const RECENT = [
  { title: "workspace shell polish", meta: "updated 3m ago", status: "live", tone: "var(--te-green)" },
  { title: "billing webhook retry", meta: "updated 18m ago", status: "warning", tone: "var(--te-yellow)" },
  { title: "permissions audit", meta: "updated 1h ago", status: "needs review", tone: "var(--te-red)" },
  { title: "docs starter refresh", meta: "updated 2h ago", status: "ready", tone: "var(--te-blue)" },
];
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {METRICS.map((metric) => (
            ...
            >
              <p className="text-[11px] tracking-[0.06em] text-foreground-tertiary">{metric.label}</p>
              <p className="mt-2 text-2xl font-light text-foreground">{metric.value}</p>
        ...
        <section className="grid gap-4 xl:grid-cols-[2fr_1fr]">
          ...
          <article className="rounded-sm border border-border bg-background-secondary p-4">
            <h2 className="mb-4 text-sm font-medium text-foreground">recent work</h2>
            <div className="space-y-2">
              {RECENT.map((item) => (
                <div key={item.title} className="rounded-sm border border-border bg-background p-3">
                  ...
                  <p className="mt-1 text-xs text-foreground-tertiary">{item.meta}</p>
                </div>
              ))}
            </div>
          </article>
