...
import { getActivePresetId, loadPreset as loadPresetLocal } from "./utils";

// Allow injecting a custom preset loader (for registry integration)
let customPresetLoader:
  | ((presetId: string) => Promise<DesignPreset | null>)
  | null = null;

export function setPresetLoader(
  loader: (presetId: string) => Promise<DesignPreset | null>
) {
...
export function PresetProvider({
  ...
  const loadAndApplyPreset = useCallback(async (presetId: string) => {
    try {
      ...
      const error =
        err instanceof Error ? err : new Error("Failed to load preset");
      setError(error);
      throw error;
    } finally {
      setLoading(false);
