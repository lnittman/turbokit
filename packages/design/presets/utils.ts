  ...
  for (const preset of additionalPresets) {
    ...
    if (preset.layers.animations) {
      composed.layers.animations = {
        ...
        settings:
          preset.layers.animations.settings ||
          composed.layers.animations?.settings,
      };
    }
...
export function generateCSSVariables(tokens: TokenLayer): string {
  ...
  if (tokens.fonts.lineHeight)
    lines.push(`  --line-height: ${tokens.fonts.lineHeight};`);
...
export function generatePresetCSS(preset: DesignPreset): string {
  ...
  if (preset.layers.tokens?.fonts.variants) {
    lines.push("");
    Object.entries(preset.layers.tokens.fonts.variants).forEach(
      ([key, variant]) => {
  ...
  if (preset.layers.animations?.keyframes) {
    lines.push("");
    Object.entries(preset.layers.animations.keyframes).forEach(
      ([name, keyframe]) => {
