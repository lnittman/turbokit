...
import type { ComposedPreset, DesignPreset, PresetRegistry } from "./schema";
import {
...
export function usePreset(): UsePresetReturn {
  ...
  const applyPreset = useCallback(async (presetId: string) => {
    try {
      ...
      setError(
        err instanceof Error ? err : new Error("Failed to apply preset")
      );
    } finally {
      setLoading(false);
    }
...
export function usePresetRegistry(): UsePresetRegistryReturn {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setError(
        err instanceof Error ? err : new Error("Failed to load registry")
      );
    } finally {
      setLoading(false);
    }
  ...
  const getPreset = useCallback(
    async (presetId: string): Promise<DesignPreset | null> => {
      try {
