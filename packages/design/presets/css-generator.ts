...
export function generatePresetCSS(preset: DesignPreset): string {
  ...
  // Note: Dark mode overrides would be generated here if preset schema supported it
  // Currently presets use CSS variables that automatically work with .dark class
...
export function generateCSSVariables(
  ...
  if (preset.layers.tokens?.colors?.semantic) {
    Object.entries(preset.layers.tokens.colors.semantic).forEach(
      ([key, value]) => {
        vars[key] = value;
      }
    );
