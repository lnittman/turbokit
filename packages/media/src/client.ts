...
import type { ImageMediaResult, MediaResult } from "./types/results";
...
export class MediaClient {
	...
	async generateWithOpenAI(input: OpenAIImageInput): Promise<ImageMediaResult> {
		...
		if (!apiKey) {
			throw new Error(
				"OPENAI_API_KEY is required. Set it in config or environment.",
			);
		}
	...
	async generateWithFal(input: FalInput): Promise<MediaResult> {
		...
		if (!apiKey) {
			throw new Error(
				"FAL_API_KEY is required. Set it in config or environment.",
			);
		}
	...
	async generateWithOpenRouter(
		...
		const apiKey =
			this.config.OPENROUTER_API_KEY || process.env.OPENROUTER_API_KEY;
		if (!apiKey) {
			throw new Error(
				"OPENROUTER_API_KEY is required. Set it in config or environment.",
			);
		}
