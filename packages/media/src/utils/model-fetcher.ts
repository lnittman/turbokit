	...
	try {
		...
		if (filters?.outputModalities) {
			models = models.filter((model: any) =>
				filters.outputModalities?.some((mod) =>
					model.architecture?.output_modalities?.includes(mod),
				),
			);
		}
...
export function getFalModelInfo(): { message: string; url: string } {
	return {
		message:
			"Fal.ai has 600+ models but does not provide a public listing API. Browse available models at:",
