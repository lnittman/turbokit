...
export class FalAdapter {
  ...
  private async detectAndParseMediaType(
    ...
    // Default: image
    return await this.parseImageResult(result, model, downloadAndEncode, timeout);
  }
  ...
  private async parseImageResult(
    ...
    if (downloadAndEncode) {
      try {
        ...
        if (!resp.ok) {
          throw new Error(`Fal.ai: fetching image failed: ${resp.status} ${resp.statusText}`);
        }

        const contentType = resp.headers.get('content-type') ?? '';
        if (!contentType.startsWith('image/')) {
          throw new Error(`Fal.ai: expected image content-type, got "${contentType}"`);
        }
