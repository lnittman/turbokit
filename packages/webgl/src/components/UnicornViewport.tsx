...
import type { UnicornSceneInstance, UnicornSDK, UnicornAddSceneOptions } from "../types/sdk"

type Interactivity = { mouse?: { disableMobile?: boolean; disabled?: boolean } }
...
export function UnicornViewport({
  ...
  const scene = useUnicornJson({ jsonPath: jsonPath ?? "", version, scale, dpi, fps: fps as ValidFPS, lazyLoad, keyPrefix, respectReducedMotion })
  ...
  useEffect(() => {
    ...
    const ensureScript = () =>
      new Promise<void>((resolve, reject) => {
        ...
        if (existing) {
          ...
          existing.addEventListener("error", () => reject(new Error("UnicornStudio script failed to load")))
          return
        }
        ...
        s.onload = () => resolve()
        s.onerror = () => reject(new Error("UnicornStudio script failed to load"))
    ...
    const init = async () => {
      ...
      // reduced motion
      const reduce = respectReducedMotion && typeof window !== 'undefined' && window.matchMedia?.('(prefers-reduced-motion: reduce)').matches
      const effFps = reduce ? Math.min(30, (fps as number) ?? scene.fps) : (fps as number) ?? scene.fps
    ...
    return () => {
      ...
      try { sceneRef.current?.destroy?.(); sceneRef.current = null } catch {}
    }
  }, [
    altText,
    ariaLabel,
  ...
  return <div id={id} ref={divRef} className={className} style={{ width, height }} role="img" aria-label={ariaLabel} />
}
