...
export async function sendFCMNotification(message: FCMMessage): Promise<boolean> {
  ...
  if (!projectId || !serviceAccountJson) {
    console.error('[FCM] Missing GOOGLE_CLOUD_PROJECT_ID or GOOGLE_SERVICE_ACCOUNT_JSON');
    return false;
  }
 ...
 */
export async function sendAPNsNotification(message: APNsMessage): Promise<boolean> {
