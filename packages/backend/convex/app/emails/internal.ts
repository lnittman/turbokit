...
export const handleEvent = internalMutation({
	...
	handler: async (ctx, { id, event }) => {
		...
		const log = async (action: string, metadata?: any) =>
		...
		switch (event.type) {
			...
			case "email.delivered":
				await log("email.delivered");
				break;
			case "email.bounced":
				await log("email.bounced", { bounceType: (event as any).bounce_type });
				break;
			case "email.complained":
				await log("email.complained");
				break;
			case "email.opened":
				await log("email.opened");
				break;
			case "email.clicked":
				await log("email.clicked", { link: (event as any).link });
				break;
		}
	},
});
