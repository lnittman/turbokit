...
export const createInitialProject = internalMutation({
  ...
  handler: async (ctx, { userId, name }) => {
    const projectId = await ctx.db.insert("projects", { name, description: "Your first project on TurboKit", ownerId: userId, status: "active", settings: { isDefault: true }, createdAt: Date.now(), updatedAt: Date.now() });
    await ctx.db.insert("teamMembers", { projectId, userId, role: "owner", joinedAt: Date.now() });
    return projectId;
  },
});
