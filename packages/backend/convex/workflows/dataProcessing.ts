...
export const dataProcessingPipeline = workflow.define({
	...
	handler: async (step, { userId, dataSourceId, processingType }) => {
		...
		const validationResult = await step.runMutation(
			anyInternal.data.validateSource,
			{ dataSourceId },
		);
		...
		switch (processingType) {
			case "analysis":
				processedData = await step.runAction(anyInternal.data.analyzeData, {
					data: extractedData,
					userId,
				});
				break;
			...
			case "transformation":
				processedData = await step.runAction(anyInternal.data.transformData, {
					data: extractedData,
					userId,
				});
				break;
			...
			case "export":
				processedData = await step.runAction(anyInternal.data.exportData, {
					data: extractedData,
					userId,
				});
				break;
		...
		const resultId = await step.runMutation(anyInternal.data.storeResults, {
			userId,
			dataSourceId,
			processingType,
		...
		await step.runAction(anyApi.app.emails.actions.sendNotificationEmail, {
			userId,
			email: await step.runQuery(anyInternal.app.users.getUserEmail, {
				userId,
			}),
...
export const batchProcessing = workflow.define({
	...
	handler: async (step, { userId, items, operation }) => {
		...
		// Store batch results
		await step.runMutation(anyInternal.data.storeBatchResults, {
			userId,
			operation,
			results,
		});
