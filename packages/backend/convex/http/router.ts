...
http.route({
  ...
  handler: httpAction(async (ctx, req) => {
    ...
    switch (event.type) {
      case "user.created":
        await ctx.runMutation(api.users.internal.createUser, {
      ...
      case "user.updated":
        await ctx.runMutation(api.users.internal.updateUser, {
          clerkId: event.data.id,
          email: event.data.email_addresses[0].email_address,
          name: event.data.first_name 
            ? `${event.data.first_name} ${event.data.last_name || ""}`.trim()
      ...
      case "user.deleted":
        await ctx.runMutation(api.users.internal.deleteUser, {
          clerkId: event.data.id,
        });
        break;
    }
