	...
	if (errorMessage) {
		...
		if (typeof errorMessage === "string") {
			if (!message.includes(errorMessage)) {
				throw new Error(
					`Expected error to include "${errorMessage}", but got "${message}"`,
				);
			}
		} else {
			if (!errorMessage.test(message)) {
				throw new Error(
					`Expected error to match ${errorMessage}, but got "${message}"`,
				);
			}
		}
	}
...
export function spyOnConsole() {
	...
	return {
		...
		getLastError: () =>
			spies.error.mock.calls[spies.error.mock.calls.length - 1],
	};
}
