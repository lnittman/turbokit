 ...
 */
export function createMockProject(ownerId: Id<"users">, overrides?: Partial<any>) {
  return {
    ...
    tags: faker.helpers.arrayElements(["development", "design", "marketing", "sales", "hr"], 3),
    createdAt: Date.now(),
    updatedAt: Date.now(),
    ...overrides,
  };
 ...
 */
export function createMockThread(userId: Id<"users">, overrides?: Partial<any>) {
  return {
 ...
 */
export function createMockMessage(threadId: Id<"threads">, overrides?: Partial<any>) {
 ...
 */
export function createMockActivity(userId: Id<"users">, overrides?: Partial<any>) {
  const actions = [
    "user.created",
    "user.updated", 
    "project.created",
  ...
  return {
    ...
    resourceType: faker.helpers.arrayElement(["user", "project", "thread", "message", "subscription"]),
    resourceId: faker.string.alphanumeric(16),
    metadata: {
      ip: faker.internet.ip(),
      userAgent: faker.internet.userAgent(),
      changes: faker.datatype.boolean() ? { before: "old", after: "new" } : undefined,
    },
    timestamp: Date.now(),
    ...overrides,
  };
...
export function createMockSubscription(userId: Id<"users">, overrides?: Partial<any>) {
  ...
  return {
    ...
    productKey,
    status: faker.helpers.arrayElement(["active", "trialing", "past_due", "cancelled"]),
    currentPeriodStart: Date.now(),
    currentPeriodEnd: Date.now() + 30 * 24 * 60 * 60 * 1000, // 30 days
    cancelAtPeriodEnd: faker.datatype.boolean(),
    trialEnd: faker.datatype.boolean() ? Date.now() + 7 * 24 * 60 * 60 * 1000 : null,
    metadata: {
      referral: faker.helpers.arrayElement([null, "google", "facebook", "friend"]),
    },
    ...overrides,
  };
}
 ...
 */
export function generateMockData(counts: {
  users?: number;
  ...
  const mockActivities = mockUsers.flatMap(user =>
    Array.from({ length: activitiesPerUser }, () => createMockActivity(user._id))
  );
