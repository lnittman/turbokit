...
export function renderWithProviders(
	...
	function Wrapper({ children }: { children: ReactNode }) {
		...
		if (withConvex) {
			wrapped = (
				<ConvexProvider client={convexClient}>{wrapped}</ConvexProvider>
			);
		}

		// Wrap with Clerk if needed
		if (withClerk) {
			wrapped = <ClerkProvider {...clerkProps}>{wrapped}</ClerkProvider>;
		}
...
export async function testA11y(container: HTMLElement) {
	const results = await import("@axe-core/react").then((mod) =>
		mod.default.run(container),
	);
...
export async function fillAndSubmitForm(
	...
	const submitButton =
		...
		Array.from(container.querySelectorAll("button")).find((btn) =>
			btn.textContent?.includes(submitButtonText),
		);
