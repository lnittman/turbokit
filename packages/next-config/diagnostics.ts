...
export function printEnvBanner(appName: string) {
  ...
  const sections: Section[] = [
    {
      label: "Auth (Clerk)",
      enabled:
        bool("NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY") &&
        (bool("CLERK_SECRET_KEY") || bool("NEXT_RUNTIME")),
    ...
    },
    {
      ...
      enabled:
        bool("NEXT_PUBLIC_POSTHOG_KEY") && bool("NEXT_PUBLIC_POSTHOG_HOST"),
    ...
    },
    {
      ...
      howToEnable:
        "Set NEXT_PUBLIC_SENTRY_DSN (optional: SENTRY_ORG, SENTRY_PROJECT)",
    },
    {
    ...
    },
    {
      ...
      howToEnable:
        "Set OPENROUTER_API_KEY (optional: OPENROUTER_BASE_URL, OPENROUTER_MODEL)",
    },
    {
  ...
  sections.forEach((s) => {
    lines.push(
      `- ${s.label}: ${s.enabled ? "enabled" : "disabled"} — ${s.details}`
    );
    if (!s.enabled) {
